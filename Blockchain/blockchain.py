...
def get_users():
    c=get_user_count()
    c_names=['username','password','name','mobile','p_address','address','gender']
    dict1={}
...
def add_user1(user_id,username,password,name,mobile,p_address,uaddress,gender): 
    ...
    tx_hash = p1.functions.addUser(user_id,username,password,name,mobile,p_address,uaddress,gender).transact()
    tx_receipt = w3.eth.getTransactionReceipt(tx_hash)
    print(tx_hash)
def add_prop(us,Prop_id,username,ptype,details,file_name,amount,status,phn,hash1): 
    print("#######:",us,Prop_id,username,ptype,details,file_name,amount,status,phn,hash1)
    f=open('contract_address.txt','r')
    address=f.read()
    f.close()
    blockchain_address = 'http://127.0.0.1:7545'
    ...
    tx_hash = p1.functions.addProp(us,Prop_id,username,ptype,details,file_name,amount,status,phn,hash1).transact()
    tx_receipt = w3.eth.getTransactionReceipt(tx_hash)
    print(tx_hash)
...
def get_prps():
    c=get_prp_count() 
    print("#############c:",c)
    c_names=['Prop_id','username','ptype','details','file_name','amount','status','phone','hash1']
    dict1={}
    for i in range(1,c+1):
        d=get_prp(i)
        print('d:',d)
...
def add_transaction_to_table(get_id,name,paddr2,nmr,paddr1,pid,amount,t_hash):
    ...
    tx_hash = p1.functions.addTransaction(get_id,name,paddr2,nmr,paddr1,pid,amount,t_hash).transact()
    tx_receipt = w3.eth.getTransactionReceipt(tx_hash)
...
def get_transactions():
    ...
    c_names=['Sender_name','Sender_address','Receiver_name','Receiver_address','Prp_id','Amount','T_hash']
    dict1={}
    for i in range(1,c+1):
        d=get_transact(i)
        dict2={}
...
def tkadd(idd,ptype,p_id,tok_name,file_name,amn,cu_owner,hashh,username): 
    ...
    tx_hash = p1.functions.addtok_prp(idd,ptype,p_id,tok_name,file_name,amn,cu_owner,hashh,username).transact()
    tx_receipt = w3.eth.getTransactionReceipt(tx_hash)
    print(tx_hash)
####################################################################################################
