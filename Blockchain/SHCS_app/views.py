...
global idd,tk_idd
s_a=open("C:/Users/akaro/anaconda3/realestate_1/Blockchain/s_id.txt","r+")
s_aa=s_a.readline()
s_a.close()
t_a=open("C:/Users/akaro/anaconda3/realestate_1/Blockchain/tk_id.txt","r+")
t_aa=t_a.readline()
t_a.close()
idd=int(s_aa)
tk_idd=int(t_aa)
...
def register(request):
	...
	obj10=Requests.objects.filter(mobile=mobile,username=username,password=password,p_address=p_address,name=name,address=address,gender=gender)
	co=obj10.count()
	if co==1:
		return HttpResponse("<script>alert('Request is in Pending list');window.location.href='/show_index/'</script>")

	else:
		obj1=Requests(mobile=mobile,username=username,password=password,p_address=p_address,name=name,address=address,gender=gender)
		obj1.save()
...
def approve(request):
	...
	if not verify_adr(p_address):
		...
		return HttpResponse("<script>alert('Public Key does not belong to blockchain');window.location.href='/show_request_admin/';</script>")
			...
			obj1=Users(username=username,password=password,name=name,address=address,gender=gender,mobile=mobile,p_address=p_address)
			obj1.save()
			obj2=Users.objects.get(username=username,password=password,name=name,address=address,gender=gender,mobile=mobile,p_address=p_address)
			user_id=obj2.r_id
			print("User id : ",user_id)
			add_user1(user_id,username,password,name,mobile,p_address,address,gender)
...
def predict_result(request):
    data = pd.read_csv(r"C:/Users/akaro/anaconda3/realestate_1/Blockchain/DataSet/USA_Housing.csv")
	...
	if check!=[]:
		...
		if(a=="Yes"):
			...
			if(c22==0 and c23==0 and c24==0):
				status="Pending"
				obj11=Selling(r_id1=idd,Name=name,Ptype=ptype,Prop_id=p_id,details=details,image=file_name,mobile=phn,amount=amount,username=username,status=status,hash1=hash1)
				obj11.save()
				...
				print("#########:",us)
				print("Type :",type(h))
				add_prop(us,h,username,ptype,details,file_name,amount,status,phn,hash1)
				###################################################################################
				se_id=str(idd)
				print("##############",se_id)
				f1=open('C:/Users/akaro/anaconda3/realestate_1/Blockchain/s_id.txt','w')
				f1.write(se_id)
				f1.close()
				###################################################################################
...
def View_selled(request):
	...
	if(s_type=="Normal"):
			...
			for k in range(0,len(hashh)):
				...
				with open(ab , 'rb') as f1:
					with open("SHCS_app/static/"+"Ipfs/"+new_file, 'wb') as f2:
						f2.write(f1.read())
				os.remove("C:/Users/akaro/anaconda3/realestate_1/Blockchain/"+ab)	
		...
		c3=bkkl.count()
		if(c3>0):
			bkkl1=Token.objects.filter(username=username).values('image','hashh')
			...
			for i in bkkl1:
				...
				with open(hashh , 'rb') as f1:
					with open("SHCS_app/static/"+"Ipfs/"+image, 'wb') as f2:
						f2.write(f1.read())
				os.remove("C:/Users/akaro/anaconda3/realestate_1/Blockchain/" +hashh)	
			################################################################
			##########################################################################
...
def Buy_p(request):
	...
	if(s_type=="Normal"):
		...
		username=request.session["username"]
		bkkl=Token.objects.filter(status="Pending").exclude(username=username)
		c3=bkkl.count()
		if(c3>0):
			bkkl1=Token.objects.filter(status="Pending").exclude(username=username).values('image','hashh')
...
def Pay(request):
	if 'uid' in request.session:
		...
		if(a=="Yes"):
			...
			print("#######",t_hash)
			obj21=Transaction(SName=name,Saddr=paddr2,RName=uname,Raddr=paddr1,P_id=pid,Amount=amount,T_hash=t_hash)
			obj21.save()
			...
			obj55=Buyed(name=uname,p_id=pid,hash1=i_hash,p_type=ptype,im=im,details=details,phone=mb)
			obj55.save()
			####################################
			obb1=Selling.objects.get(Prop_id=pid)
			obb1.delete()
			####################################
			ob=Transaction.objects.get(SName=name,Saddr=paddr2,RName=uname,Raddr=paddr1,P_id=pid,Amount=amount,T_hash=t_hash)
			get_id=ob.r_id1
			add_transaction_to_table(get_id,name,paddr2,uname,paddr1,pid,amount,t_hash)
...
def buyprp(request):
	if 'uid' in request.session:
		...
		if(s_type=="Normal"):
			...
			c3=bkkl.count()
			if(c3>0):
				bkkl1=Buyed_tk.objects.filter(name=name).values('im','hash1')
				bkkl1=list(bkkl1)
				print("bkkl1########:",bkkl1)
				##########################################################################
				if os.path.isdir("SHCS_app/static/"+"Ipfs"):		
	...
	if(a=="Yes"):
		...
		if(c22==0 and c23==0):
			for i in range(1,token+1):
				...
				blk1=Token(r_id1=tk_idd,Name=name,Ptype=ptype,Prop_id=p_id,details=details,tkname=tok_name,image=file_name11,mobile=phn,amount=amnt,status=status,username=username,token_count=token,Cuholder=Cuholder,hashh=hashh,Button=button)
				blk1.save()	
				##################################################################################################################################################################################################################
				...
				print(idd1)
				##################################################################################################################################################################################################################
				tkadd(idd1,ptype,p_id,tok_name,file_name11,amn,Cuholder,hashh,username)#to store tokens to blockchain
			################################################################################################
			tk_id=str(tk_idd)
			f1=open('C:/Users/akaro/anaconda3/realestate_1/Blockchain/tk_id.txt','w')
			f1.write(tk_id)
			f1.close()
...
def b_sell(request):
	...
	if(a=="Yes"):
		...
		if(c22==0):
			...
			####################################
			status="Pending"
			obj11=Selling(r_id1=idd,Name=name,Ptype=ptype,Prop_id=p_id,details=details,image=img,mobile=mob,amount=amount,username=username,status=status,hash1=hashh)
			obj11.save()
			...
			print("#########:",us)
			print("Type :",type(h))
			add_prop(us,h,username,ptype,details,img,amount,status,mob,hashh)
			obb=Buyed.objects.get(p_id=p_id)
			obb.delete()
			###################################################################################
			se_id=str(idd)
		...
		if(a=="Yes"):
			...
			if(c22==0 and c23==0 and c24==0):
				for i in range(1,token+1):
					...
					tok_name=p_id+"__"+"token"+str(i)
					file_name11=p_id+"__"+"token"+str(i)+".jpg"
					blk1=Token(r_id1=tk_idd,Name=name,Ptype=ptype,Prop_id=p_id,details=details,tkname=tok_name,image=file_name11,mobile=phn,amount=amnt,status=status,username=username,token_count=token,Cuholder=Cuholder,hashh=hash1,Button=button)
					blk1.save()	
					##################################################################################################################################################################################################################
					obj323=Token.objects.get(tkname=tok_name)
					amn=str(obj323.amount)
...
def pay_tk(request):
	if 'uid' in request.session:
		...
		if(a=="Yes"):
			t_hash=transfer(paddr1,paddr2,pvt,amount)
			print("#######",t_hash)
			obj21=Transaction(SName=cuname,Saddr=paddr2,RName=nmr,Raddr=paddr1,P_id=tkna,Amount=amount,T_hash=t_hash)
			obj21.save()
			obj22=Token.objects.get(tkname=tkna)
			obj22.delete()
			ob=Transaction.objects.get(SName=cuname,Saddr=paddr2,RName=nmr,Raddr=paddr1,P_id=tkna,Amount=amount,T_hash=t_hash)
			get_id=ob.r_id1
			add_transaction_to_table(get_id,cuname,paddr2,nmr,paddr1,tkna,amount,t_hash)
			obj55=Buyed_tk(name=nmr,p_id=pid,tk_name=tkna,hash1=hashh,p_type=ptype,im=img,details=details,amnt=amount,phone=mb,tok_co=tco)
			obj55.save()
			return HttpResponse("<script>alert('Payment Successful');window.location.href='/Transactin_tab/';</script>")
		else:
			return HttpResponse("<script>alert('Invalid Private Key');window.location.href='/Buy/';</script>")
...
def b_sell_tk(request):
	...
	if 'uid' in request.session:
		...
		print(a)
		if(a=="Yes"):
			blk1=Token(r_id1=tk_idd,Name=namee,Ptype=p_type,Prop_id=pr_id,details=details,tkname=tk_nm,image=img,mobile=mbb,amount=amount,status=status,username=uname,token_count=tok_co,Cuholder=namee,hashh=hashhh,Button=button)
			blk1.save()	
			##################################################################################################################################################################################################################
			...
			print(idd1)
			##################################################################################################################################################################################################################
			tkadd(idd1,p_type,pr_id,tk_nm,img,amount,namee,hashhh,uname)	#to store tokens to blockchain		
	...
	if(a=="Yes"):
		...
		if(c22==0):
			...
			try:
				...
				print("**")
			####################################
			obj11=Selling(r_id1=idd,Name=name,Ptype=ptype,Prop_id=p_id,details=details,image=img,mobile=mobile,amount=amount,username=uname,status=status,hash1=hashh)
			obj11.save()
			obj32=Selling.objects.get(Prop_id=p_id,status="Pending")
			h=obj32.Prop_id
			us=obj32.r_id1
			...
			print("#########:",us)
			print("Type :",type(h))
			add_prop(us,h,uname,ptype,details,img,amount,status,mobile,hashh)
			###################################################################################
			se_id=str(idd)
			f1=open('C:/Users/akaro/anaconda3/realestate_1/Blockchain/s_id.txt','w')
			f1.write(se_id)
